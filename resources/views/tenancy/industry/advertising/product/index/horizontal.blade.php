    ...
    <div class="container mx-auto px-8 py-32">
        ...
        <div class="grid gap-4 lg:gap-8 lg:grid-cols-2 xl:grid-cols-3">
            @foreach($products as $product)
                <div class="group @if(true) card @else item @endif p-0 overflow-hidden flex">

                    <a href="{{ route('product.show', $product->id) }}"
                       class="aspect-square bg-base-100 relative flex items-center justify-center overflow-hidden">
                        <img src="{{ $product->primary_image_url }}" alt="{{ $product->name }}"
                             class="object-cover w-80 transition-transform duration-300 group-hover:scale-105"
                             loading="lazy">
                    </a>
                    ...
                    <div class="p-4 flex flex-col gap-3 justify-between w-full">
                        ...
                        <a href="{{ route('product.show', $product->id) }}" class="btn btn-primary py-1 px-3 w-fit">
